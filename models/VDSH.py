...
class VDSH(nn.Module):
    ...
    def get_binary_code(self, train, test):
        ...
        if self.hascuda:
            ...
            train_b = (train_z > mid_val).type(torch.ByteTensor)
            test_b = (test_z > mid_val).type(torch.ByteTensor)
    ...
    def get_code(self, sample):
        return self.encode(sample.to(self.device))[0]
